...
- Last commit: `docs(specs): draft spec 004 and propose the model registry`
...
- Spec 004 (Codeforces collector) is in `draft` with the model registry proposal. It waits for owner ratification.
...
1. Owner: ratify spec 004 and the model registry (merge the open PR), then implementation starts.
